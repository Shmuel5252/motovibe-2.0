...
## נספח — בעיות שזוהו, סטטוס תיקון

- **✅ תוקן** — אי-עקביות בשדה תגובת ולידציה: `auth.controller.js` החזיר `detail` (יחיד) בניגוד לשאר הקונטרולרים (`details`, רבים). תוקן לשימוש ב-`sendValidationError` המשותף. בנוסף נמצא ותוקן **הבאג האמיתי שזה הסווה**: `useAuth.js` קרא להודעת שגיאה מהנתיב הלא-נכון (`error.response.data.message` במקום `error.response.data.error.message`), כך שהודעות שגיאה ספציפיות (כמו "משתמש עם אימייל זה כבר קיים") מעולם לא הוצגו למשתמש בפועל. נבדק end-to-end על שרת רץ.

- **✅ תוקן** — טיפול בשגיאות API לא אחיד בין `useBikes.js`/`useRoutes.js`/`useHistory.js`: אוחדו להתנהגות עקבית — כל קריאת API מתנתקת אוטומטית ב-401 (`handleUnauthorized`), וכולן מציגות את הודעת השגיאה מהשרת כשקיימת, עם fallback קבוע כשאין. זה היה שינוי התנהגות מכוון (לא רק רפקטור מבני) — בוצע לפי בקשה מפורשת. הועבר ל-`app/utils/apiError.js` משותף.
