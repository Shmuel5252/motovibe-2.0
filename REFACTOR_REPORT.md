...
## נספח — בעיות שזוהו אך לא תוקנו בשלב זה (יתועד, לא יתוקן "בדרך")

- **אי-עקביות בשדה תגובת ולידציה:** `auth.controller.js` מחזיר `detail` (יחיד) בעוד שאר הקונטרולרים מחזירים `details` (רבים, אוחדו ב-`server/src/app/utils/validationResponse.js`). אם קוד בלקוח (`useAuth.js` וכו') קורא ספציפית ל-`error.detail` במקום `error.details` בהתאם לקריאת ה-endpoint, זהו לא רק חוסר עקביות מבנית אלא **באג פוטנציאלי** בטיפול בשגיאות ולידציה של הרשמה/התחברות. יש לבדוק את `useAuth.js` בשלב התיקון (לא כחלק מרפקטור מבני) ולתקן את ההתנהגות בקומיט נפרד ומודע.

- **טיפול בשגיאות API לא אחיד בין hooks (לא אוחד בכוונה — ראו הסבר):** `useBikes.js` (`handleApiError`), `useRoutes.js` (`fetchRoutesFromServer`) ו-`useHistory.js` (`fetchHistoryFromServer`) מטפלים בשגיאת 401/הודעת שגיאה בשלוש דרכים **שונות בהתנהגות**, לא רק בקוד:
  - `useBikes.js`: קורא ל-`handleUnauthorized()` בכל 401, ומציג את הודעת השרת בפועל (`error.response.data.error.message`) אם קיימת, ורק אם לא — fallback קבוע.
  - `useRoutes.js`: קורא ל-`handleUnauthorized()` בכל 401, אך **לא** מציג הודעת שרת בכלל — תמיד fallback קבוע ("טעינת מסלולים נכשלה").
  - `useHistory.js`: **לא** קורא ל-`handleUnauthorized()` כלל גם ב-401, ומציג fallback קבוע בלבד תוך לוג ל-console.
  
  מכיוון שאלה הבדלי **התנהגות** בפועל (לא רק כפילות קוד צורנית), איחוד שלהם לפונקציה משותפת אחת ידרוש להחליט איזו התנהגות היא ה"נכונה" — וזו החלטת מוצר/לוגיקה, לא רפקטור מבני טהור. מומלץ קודם להחליט במודע (1) האם כל קריאת API צריכה להתנתק אוטומטית מהמשתמש ב-401, ו-(2) האם להציג למשתמש את הודעת השגיאה מהשרת או טקסט קבוע — ואז לאחד בקומיט נפרד שמתעד את שינוי ההתנהגות בפועל.
