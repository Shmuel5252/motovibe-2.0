...
function UnifiedSystemPanel({
  ...
  return (
    <div className="relative w-full shrink-0 mt-2 mb-2">
      ...
      <div className="absolute inset-0 bg-linear-to-br from-emerald-500/20 to-teal-500/10 blur-3xl rounded-full translate-y-2 scale-105 pointer-events-none" />

      <div
        className="w-full bg-white/4 border border-white/10 backdrop-blur-xl rounded-[1.25rem] p-4 flex flex-col gap-4 shadow-[0_10px_40px_rgba(0,0,0,0.5)] relative overflow-hidden transition-all duration-200"
        dir="rtl"
      >
        {/* Subtle inner emerald reflection */}
        <div className="absolute top-0 inset-x-0 h-px bg-linear-to-r from-transparent via-emerald-500/30 to-transparent opacity-50" />

        {/* Minimal Status Row - Telemetry feeling */}
        <div className="flex w-full items-center justify-between px-2 relative z-10 border-b border-white/4 pb-3">
          ...
          <div className="flex flex-col items-center gap-1 transition-all duration-200">
            ...
            <span className="text-[11px] font-medium text-white/90 truncate max-w-20">
        ...
        <div className="flex w-full p-1 bg-black/30 rounded-xl border border-white/5 shadow-inner relative z-10">
          <button
            type="button"
            onClick={onFreeride}
            className={`flex flex-1 items-center justify-center gap-2 rounded-lg py-3 transition-all duration-200 ease-out active:scale-95 ${
              freerideActive
                ? "bg-white/10 text-white font-medium shadow-sm border border-white/5"
                : "bg-transparent text-white/40 hover:text-white/70 hover:bg-white/2"
            }`}
          >
...
function StartRideButton({ onClick, error }) {
  return (
    <div className="w-full shrink-0 relative z-10">
      {error && (
        <p
          className="mb-2 flex items-center justify-center gap-1.5 text-xs font-semibold text-rose-400"
          dir="rtl"
        >
      ...
      <button
        type="button"
        onClick={onClick}
        className="group relative w-full overflow-hidden rounded-[1.25rem] py-4 transition-all duration-200 ease-out hover:scale-105 active:scale-95 bg-linear-to-r from-emerald-600 to-emerald-400 shrink-0"
        style={{
      ...
      >
        ...
        <div className="flex flex-col items-center justify-center relative z-10">
          <span className="text-[17px] font-bold tracking-widest text-[#001a0e] group-hover:drop-shadow-[0_0_8px_rgba(255,255,255,0.4)] transition-all duration-200">
            התחל רכיבה
          </span>
        </div>
      </button>
...
function ExternalNavigation() {
  return (
    <div
      className="w-full grid grid-cols-2 gap-3 shrink-0 relative z-10"
      dir="rtl"
    >
      {/* Waze */}
      <button
        onClick={() =>
          window.open("waze://?ll=31.0461,34.8516&navigate=yes", "_blank")
        }
      ...
      >
        <Navigation
          size={15}
          strokeWidth={1.5}
          className="text-emerald-400 group-hover:drop-shadow-[0_0_10px_rgba(52,211,153,0.5)] transition-all duration-200"
        />
        <span className="text-[13px] text-white/80 font-medium tracking-wide group-hover:text-white transition-colors duration-200">
          Waze
        </span>
      </button>
...
export default function RideControlCenter({
  ...
  return (
    <div className="flex h-dvh w-full flex-col overflow-hidden relative bg-transparent">
      ...
      <div className="flex-none relative w-full h-70 p-5 pb-0">
        <MapPreview
