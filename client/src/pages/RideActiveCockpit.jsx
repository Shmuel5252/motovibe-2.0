...
function LiveWeatherChip({ weather }) {
    ...
    return (
        <div className="flex items-center gap-1.5 rounded-sm border border-white/10 px-2.5 py-1.5 backdrop-blur-md shadow-sm" style={{ background: "rgba(13, 17, 23, 0.7)" }}>
            {weather?.isLoading && !weather?.error ? (
                <Loader2 size={13} className="text-slate-400 animate-spin" strokeWidth={1.5} />
            ) : (
                <WeatherIcon size={13} className={weather?.error ? "text-slate-500" : iconColor} strokeWidth={1.5} />
            )}
            <span className="text-[11px] font-bold tracking-widest text-white/90 uppercase" dir="ltr" style={{ fontVariantNumeric: "tabular-nums" }}>
...
function StatusBar({ rideElapsedSeconds, gpsAccuracyPct, onMinimize, weather }) {
    return (
        <div className="flex w-full items-start justify-between px-5 pt-4 pb-0 sm:px-6">
            ...
            <div className="flex flex-col gap-2 items-end">
                <div
                    ...
                    style={{ borderColor: "#1e293b", background: "rgba(13, 17, 23, 0.7)", backdropFilter: "blur(8px)" }}
                >
                    <Navigation size={12} strokeWidth={2} style={{ color: gpsAccuracyPct !== null ? NEON : "#475569" }} />
                    <span
                ...
                <div className="flex items-center gap-2">
                    ...
                    <span className="relative flex h-2 w-2">
                        <span
                            ...
                            style={{ backgroundColor: NEON, animation: "pulse 3s cubic-bezier(0.4, 0, 0.6, 1) infinite" }}
                        />
                        <span
...
function Speedometer({ speedKmh, maxSpeedKmh, rideElapsedSeconds }) {
    ...
    const filledArc = describeArc(GAUGE_CX, GAUGE_CY, GAUGE_R, START_ANGLE, speedAngle);
    const bgArc = describeArc(GAUGE_CX, GAUGE_CY, GAUGE_R, START_ANGLE, START_ANGLE + SWEEP);
    ...
    return (
        <div className="flex w-full flex-col items-center justify-center">
            ...
            <div className="relative w-full max-w-[290px] sm:max-w-[320px] aspect-[240/190] transition-all duration-300 ease-out">
                <svg
                    viewBox="0 0 240 190"
                    width="100%"
                    height="100%"
                ...
                >
                    ...
                    <text
                        ...
                        className="transition-all duration-300 ease-out"
                        style={{ fontVariantNumeric: "tabular-nums", textShadow: "0 2px 10px rgba(0,0,0,0.5)" }}
                    >
                        {speedKmh}
                    </text>
                    <text
...
function TelemetryCell({ icon: Icon, value, unit, label }) {
    return (
        <div className="flex flex-1 flex-col items-center justify-center gap-1.5 px-2 py-4">
            <Icon size={14} className="text-emerald-400 opacity-80" strokeWidth={1.5} />
            <div className="flex items-baseline gap-0.5 mt-1">
                ...
                {unit ? (
                    <span className="text-[10px] font-medium tracking-wide text-slate-400">{unit}</span>
                ) : null}
            </div>
...
function TelemetryHUD({ totalDistanceKm, rideElapsedSeconds, currentSpeedKmh }) {
    ...
    return (
        ...
        <div
            ...
            style={{
                ...
                boxShadow: "inset 0 1px 0 rgba(255,255,255,0.08), inset 0 0 20px rgba(0,0,0,0.5), 0 4px 20px rgba(0,0,0,0.2)",
            }}
        >
            <TelemetryCell icon={Route} value={totalDistanceKm.toFixed(1)} unit="km" label="מרחק" />

            {/* Elegant soft divider */}
            <div className="my-4 w-px self-stretch bg-gradient-to-b from-transparent via-white/10 to-transparent" />

            <TelemetryCell icon={Timer} value={formatMinutes(rideElapsedSeconds)} unit="" label="זמן" />

            {/* Elegant soft divider */}
            <div className="my-4 w-px self-stretch bg-gradient-to-b from-transparent via-white/10 to-transparent" />

            <TelemetryCell icon={TrendingUp} value={avgSpeed} unit="kmh" label="ממוצע" />
        </div>
    );
}
...
function MapControls({ mapInstance, myLocation, containerClassName }) {
    ...
    return (
        <div className={containerClassName || "absolute right-3 bottom-4 z-10 flex flex-col items-center gap-1.5 pointer-events-auto"}>
            ...
            >
                ...
                <div className="h-px w-5" style={{ background: "rgba(100,116,139,0.45)" }} />
                <button
                    type="button"
                    aria-label="הקטן מפה"
                    onClick={() => handleZoom(-1)}
            ...
            {/* 4 — Locate Me */}
            <MapCtrlBtn onClick={handleLocate} isActive={locateActive} ariaLabel="מיקום נוכחי">
...
const DARK_MAP_STYLES = [
    ...
    { featureType: "road", elementType: "geometry", stylers: [{ color: "#162133" }] },
...
export function MapPreview({ isMapLoaded, mapLoadError, selectedRoute, recordedPath, customContainerClass, customContainerStyle, innerClassName = "rounded-3xl overflow-hidden border border-white/10 shadow-lg relative w-full h-full", controlsContainerClassName, children }) {
    ...
    }, [isMapLoaded, selectedRoute?._id || selectedRoute?.id || selectedRoute?.title]);
    ...
    return (
        ...
        >
            ...
            <div
                ...
                style={{
                    boxShadow: "inset 0 4px 30px rgba(0,0,0,0.5), 0 4px 24px rgba(0,0,0,0.4)",
                    background: "rgba(10,15,30,0.5)",
                }}
            >
                {mapLoadError ? (
                    <div className="flex h-full min-h-[130px] items-center justify-center bg-slate-900 text-xs text-slate-500">
                        Map unavailable
                    </div>
                ) : !isMapLoaded ? (
                    <div className="flex h-full min-h-[130px] items-center justify-center bg-slate-900">
            ...
            <MapControls mapInstance={mapInstance} myLocation={myLocation} containerClassName={controlsContainerClassName} />
        </div>
    );
}
...
function ActionBar({ isRidePaused, setIsRidePaused, onFinish, stopError, selectedRoute }) {
    ...
    return (
        <>
            {/* Navigate sheet */}
            {showNav && (
                <NavigateSheet selectedRoute={selectedRoute} onClose={() => setShowNav(false)} />
            )}

            {stopError && (
                <p className="mb-2 px-5 text-center text-xs font-semibold text-rose-400" dir="rtl">
                    {stopError}
                </p>
            )}
            ...
            <div className="flex-1 flex flex-col justify-center relative z-10 w-full px-1">
                ...
                {selectedRoute && (
                    <div
                        ...
                        dir="rtl"
                    >
                        <div className="h-4 w-0.5 rounded-full" style={{ background: NEON, opacity: 0.7 }} />
                        <p className="truncate text-xs font-bold text-white">{selectedRoute.title}</p>
